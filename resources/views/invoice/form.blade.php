        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <div>
                                <h1 class="text-2xl font-bold text-white">Nieuwe Factuur</h1>
                                <p class="text-blue-100 text-sm">Vul de gegevens in voor een professionele PDF</p>
                            </div>
                        </div>
                    </div>
                </div>

                <form action="{{ route('invoice.generate') }}" method="POST" class="p-8" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div>
                            <label for="invoice_number" class="block text-sm font-medium text-slate-600 mb-1.5">Factuurnummer</label>
                            <input type="text" id="invoice_number" name="invoice_number" value="{{ old('invoice_number', $nextInvoiceNumber ?? '') }}"
                                   class="w-full border border-slate-200 rounded-lg px-4 py-2.5 bg-slate-50 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                                   placeholder="FAC0001" readonly>
                            <p class="text-xs text-slate-400 mt-1">Automatisch gegenereerd</p>
                        </div>

                        <div>
                            <label for="invoice_date" class="block text-sm font-medium text-slate-600 mb-1.5">Factuurdatum *</label>
                            <input type="date" id="invoice_date" name="invoice_date" value="{{ old('invoice_date', date('Y-m-d')) }}"
                                   class="w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" required>
                        </div>

                        @php
                            $selectedPaymentTerms = old('payment_terms', $duplicateData['payment_terms'] ?? '30');
                            $selectedVatRate = old('vat_rate', $duplicateData['vat_rate'] ?? '21');
                        @endphp

                        <div>
                            <label for="payment_terms" class="block text-sm font-medium text-slate-600 mb-1.5">Betalingstermijn</label>
                            <select id="payment_terms" name="payment_terms"
                                    class="w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all bg-white">
                                <option value="14" {{ $selectedPaymentTerms == '14' ? 'selected' : '' }}>14 dagen</option>
                                <option value="30" {{ $selectedPaymentTerms == '30' ? 'selected' : '' }}>30 dagen</option>
                                <option value="60" {{ $selectedPaymentTerms == '60' ? 'selected' : '' }}>60 dagen</option>
                                <option value="direct" {{ $selectedPaymentTerms == 'direct' ? 'selected' : '' }}>Direct</option>
                            </select>
                        </div>
                    ...
                    <div class="mb-8">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center">
                                <div class="w-9 h-9 bg-gradient-to-br from-purple-100 to-pink-100 rounded-lg flex items-center justify-center mr-3">
                                    <svg class="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
                                    </svg>
                                </div>
                                <h2 class="text-lg font-bold text-slate-900">Factuurregels</h2>
                            </div>
                            <button type="button" id="add-row-btn"
                                    class="inline-flex items-center px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 font-medium rounded-lg transition-colors text-sm">
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="notes" name="notes" rows="3"
                                  class="w-full border border-slate-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none"
                                  placeholder="Eventuele extra opmerkingen voor de factuur...">{{ old('notes', $duplicateData['notes'] ?? '') }}</textarea>
                    </div>

                    <!-- Action Buttons -->
                    <div class="flex flex-col sm:flex-row gap-4 pt-6 border-t border-slate-100">
                        <button type="submit" id="submit-btn"
                                class="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-4 px-6 rounded-xl transition-all duration-200 shadow-lg shadow-blue-200 hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center group">
                            ...
                            <span id="submit-text">Genereer PDF Factuur</span>
                        </button>
            ...
            <div class="mt-8 bg-white/60 backdrop-blur-sm rounded-2xl border border-slate-200/50 p-6">
                <div class="flex items-start">
                    ...
                    <div class="ml-4">
                        <h3 class="text-sm font-bold text-slate-900 mb-2">Tips voor je factuur</h3>
                        <ul class="text-sm text-slate-600 space-y-1.5">
                            ...
                            <li class="flex items-center">
                                ...
                                BTW wordt automatisch berekend (21% Nederlands tarief)
                            </li>
                            <li class="flex items-center">
                                <svg class="w-4 h-4 text-emerald-500 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('form').addEventListener('submit', function(e) {
                ...
                text.textContent = 'Bezig met genereren...';
            });

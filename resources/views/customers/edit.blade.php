    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-bold text-slate-900">Klant Bewerken</h1>
                <p class="text-slate-600 mt-1">Pas de gegevens van {{ $customer->name }} aan</p>
            </div>
                ...
                <form action="{{ route('customers.update', $customer) }}" method="POST" class="p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3"
                            class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                            placeholder="Eventuele opmerkingen over deze klant...">{{ old('notes', $customer->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>

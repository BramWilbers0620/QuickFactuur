<x-app-layout>
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 p-4 mb-6">
                <form method="GET" action="{{ route('quotes.index') }}" class="flex flex-col lg:flex-row gap-4">
                    <!-- Search Input -->
                    <div class="flex-1 min-w-0">
                        <div class="relative">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Zoek op offertenummer, klant of email..." class="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors">
                        </div>
                    </div>

                    <!-- Status Filter -->
                    <div class="flex-shrink-0 w-full lg:w-48">
                        <select name="status" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors">
                    ...
                    <div class="flex-shrink-0 flex gap-2">
                        <input type="date" name="date_from" value="{{ request('date_from') }}" placeholder="Van" class="w-full lg:w-36 px-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors text-sm">
                        <input type="date" name="date_to" value="{{ request('date_to') }}" placeholder="Tot" class="w-full lg:w-36 px-3 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors text-sm">
                    </div>

                    <!-- Buttons -->
                    <div class="flex-shrink-0 flex gap-2">
                        <button type="submit" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-xl transition-colors">
                            Zoeken
                        </button>
                        @if(request()->hasAny(['search', 'status', 'date_from', 'date_to']))
                            <a href="{{ route('quotes.index') }}" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-xl transition-colors">
                                Reset
                            </a>
                        @endif
                    </div>
            ...
            @if($quotes->isEmpty() && !request()->hasAny(['search', 'status', 'date_from', 'date_to']))
                <!-- Empty State -->
            ...
            @elseif($quotes->isEmpty())
                ...
                <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-12 text-center">
                    ...
                    <h3 class="text-2xl font-bold text-slate-900 mb-3">Geen offertes gevonden</h3>
                    <p class="text-slate-600 mb-8 max-w-md mx-auto">Er zijn geen offertes die voldoen aan je zoekcriteria. Probeer andere filters.</p>
                    <a href="{{ route('quotes.index') }}" class="inline-flex items-center px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-xl transition-colors">
                        Filters wissen
                    </a>
                </div>
            @else
                                        ...
                                        <td class="px-6 py-5 whitespace-nowrap text-right">
                                            <div class="flex items-center justify-end space-x-2">
                                                ...
                                                @if($quote->customer_email)
                                                    <button onclick="sendQuoteEmail(this, {{ $quote->id }})" class="inline-flex items-center px-3 py-1.5 bg-purple-50 hover:bg-purple-100 text-purple-600 text-sm font-medium rounded-lg transition-colors" title="Verstuur per e-mail">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        function sendQuoteEmail(button, quoteId) {
